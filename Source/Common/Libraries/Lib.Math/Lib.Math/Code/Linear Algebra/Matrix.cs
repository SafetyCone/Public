...
namespace Public.Common.Lib.Math
{
    ...
    public class Matrix
    {
        ...
        public static void DotProductFromRightInPlace(Matrix m, Vector rightVector, Vector output)
        {
            for (int iRow = 0; iRow < m.Rows; iRow++)
            {
                double outputValue = 0;
                for (int iCol = 0; iCol < m.Columns; iCol++)
                {
                    double curValue = m.Values[iRow, iCol] * rightVector.Values[iCol];
